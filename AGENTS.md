...
- Multi-leader manifests are strict schema-1 YAML loaded only beneath `manifest_root`. `write_manifest` validates and persists agent-generated YAML under that root. `replicate_config_manifest` snapshots the source once, honors manifest concurrency by default, and reports item-level aggregate progress; `validate_config_manifest` has pageable difference/all-item detail with create/update/noop splits and treats environment identity drift as non-blocking; `commit_and_deploy_manifest` requires a durable apply receipt and includes affected Edge descendants.
