...
- Mutation execution returns a durable `job_id`; poll `get_config_deployment_job` for its bounded final result or add `target` for per-target detail. SQLite-backed jobs survive process restarts as `interrupted`, run off the MCP event loop, and are serialized per configured server so status/diff reads remain responsive.
- Multi-leader manifests are strict schema-1 YAML loaded only beneath `manifest_root`. `replicate_config_manifest` snapshots the source once and fans out with bounded concurrency; `validate_config_manifest` treats environment identity drift as non-blocking; `commit_and_deploy_manifest` requires a durable apply receipt and includes affected Edge descendants.
