...
- Fleet declarations accept `isSearch` and `streamtags`, never deployment state (`configVersion`, `lookupDeployments`). Manifest retries may reuse only same-intent receipts whose complete diffs, fleet existence, and target-local versions still match. Only receipt-created parents may bypass undeployed-state blockers; conflicts, behind state, and external ancestors remain guarded. Successful manifest pushes report API readback separately from push success because Cribl may cache ahead counts; never claim independently verified remote sync from that readback.
