...
- Thirty-seven tools: get_leader_overview, get_edge_info, list_groups, list_sources, list_destinations, list_pipelines, list_routes, list_breakers, list_lookups, list_variables, list_packs, get_pack, install_pack, upload_pack, update_pack, delete_pack, get_config_objects, validate_config_objects, copy_resource_config, validate_resource_sync, sync_user, replicate_group_config, validate_group_config, replicate_system_settings, validate_system_settings, write_manifest, replicate_config_manifest, validate_config_manifest, commit_and_deploy_manifest, get_group_git_status, get_group_git_diff, get_config_deployment_job, commit_group_config, deploy_group_config, commit_and_deploy_group, commit_and_deploy_all, push_config_git
...
- Schema-1 YAML manifests live beneath the configured `manifest_root`, contain only configured leader names and explicit group/kind/item IDs, and reject aliases, duplicate keys, unknown fields, path escapes, and inline payloads. `write_manifest` closes the sandboxed-agent authoring loop with validate-before-write and explicit overwrite protection. `replicate_config_manifest` snapshots the source once, uses manifest concurrency unless explicitly overridden, reports item-level work across targets, and emits a durable apply receipt. `validate_config_manifest` ignores identity-only drift, splits create/update/noop counts, and pages difference or all-item detail. `commit_and_deploy_manifest` requires that receipt and commits only manifest groups plus affected Edge descendants.
