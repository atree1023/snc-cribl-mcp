"""Unit tests for the list_routes MCP tool wrapper.
...
from snc_cribl_mcp.operations.routes import collect_product_routes as _collect_product_routes
from snc_cribl_mcp.tools.list_routes import register as register_list_routes
...
@pytest.mark.asyncio
async def test_list_routes_tool_success(deps_base: SimpleNamespace, mock_ctx: Context) -> None:
    """The tool should aggregate routes across products and return formatted JSON."""
    ...
    resp_e1.items[0].model_dump.return_value = {"name": "r2"}
    resp_e1.items[1].model_dump.return_value = {"name": "r3"}

    async def routes_list_async(*_args: object, **kwargs: dict[str, object]) -> MagicMock:
        srv_url = str(kwargs.get("server_url", ""))
        assert srv_url.endswith(("/m/g1", "/m/e1"))
        return resp_g1 if srv_url.endswith("/m/g1") else resp_e1

    mock_client.routes = MagicMock(list_async=AsyncMock(side_effect=routes_list_async))
    ...
    deps = SimpleNamespace(
        ...
        collect_product_routes=_collect_product_routes,
    )

    app = _FakeApp()
    register_list_routes(app, deps=deps)  # type: ignore[arg-type]
    assert "list_routes" in app.tools

    raw = await app.tools["list_routes"](mock_ctx)
    data = raw

    routes = cast("dict[str, Any]", data["routes"])

    assert data["base_url"] == deps_base.config.base_url_str
    assert "stream" in routes
    assert "edge" in routes
    stream = cast("dict[str, Any]", routes["stream"])
    edge = cast("dict[str, Any]", routes["edge"])
    assert stream["total_count"] == 1
    assert edge["total_count"] == 2


@pytest.mark.asyncio
async def test_list_routes_tool_handles_unavailable_product(
    ...
    mock_client.routes = MagicMock(list_async=AsyncMock(return_value=empty_resp))
