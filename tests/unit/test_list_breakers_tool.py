"""Unit tests for the list_breakers MCP tool wrapper.
...
from snc_cribl_mcp.operations.breakers import collect_product_breakers as _collect_product_breakers
from snc_cribl_mcp.tools.list_breakers import register as register_list_breakers
...
@pytest.mark.asyncio
async def test_list_breakers_tool_success(deps_base: SimpleNamespace, mock_ctx: Context) -> None:
    """The tool should aggregate breakers across products and return formatted JSON."""
    ...
    # Mock async_client for breakers
    ...
    resp_e1.json.return_value = {"items": [{"id": "b2"}, {"id": "b3"}], "count": 2}

    async def http_get(url: str, **_kwargs: object) -> MagicMock:
        if "/m/g1/lib/breakers" in url:
            return resp_g1
        if "/m/e1/lib/breakers" in url:
            return resp_e1
    ...
    deps = SimpleNamespace(
        **deps_base.__dict__,
        ...
        collect_product_breakers=_collect_product_breakers,
    )

    app = _FakeApp()
    register_list_breakers(app, deps=deps)  # type: ignore[arg-type]
    assert "list_breakers" in app.tools

    raw = await app.tools["list_breakers"](mock_ctx)
    data = raw

    breakers = cast("dict[str, Any]", data["breakers"])

    assert data["base_url"] == deps_base.config.base_url_str
    assert "stream" in breakers
    assert "edge" in breakers
    stream = cast("dict[str, Any]", breakers["stream"])
    edge = cast("dict[str, Any]", breakers["edge"])
...
@pytest.mark.asyncio
async def test_list_breakers_tool_handles_unavailable_product(
