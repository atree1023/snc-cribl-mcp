"""Unit tests for route collection helpers.
...
from snc_cribl_mcp.operations.routes import collect_product_routes
...
class TestSerializeModel:
    ...
    def test_serialize_model_with_pydantic_model(self) -> None:
        ...
        mock_obj = MagicMock()
        mock_obj.model_dump.return_value = {"id": "test", "name": "route1"}

        result = serialize_model(mock_obj)

        assert result == {"id": "test", "name": "route1"}
...
@pytest.mark.asyncio
async def test_collect_product_routes_success(mock_ctx: Context) -> None:
    """It should list routes for each group via the product-scoped client and aggregate results."""
    ...
    stream_routes = MagicMock()
    mock_client.routes = stream_routes

    resp_g1 = MagicMock(items=[MagicMock(), MagicMock()], count=2)
    resp_g1.items[0].model_dump.return_value = {"name": "r1"}
    resp_g1.items[1].model_dump.return_value = {"name": "r2"}

    resp_g2 = MagicMock(items=[MagicMock()], count=1)
    resp_g2.items[0].model_dump.return_value = {"name": "r3"}
    ...
    stream_routes.list_async = AsyncMock(side_effect=list_async_side_effect)

    result = await collect_product_routes(
        mock_client,
...
@pytest.mark.asyncio
async def test_collect_product_routes_404_per_group(mock_ctx: Context) -> None:
    """404 on a group's routes should be treated as empty for that group."""
    mock_client = MagicMock()
    ...
    stream_routes.list_async = AsyncMock(side_effect=api_error_404)
...
@pytest.mark.asyncio
async def test_collect_product_routes_api_error_non_404(mock_ctx: Context) -> None:
    ...
    mock_client.sdk_configuration = MagicMock(server_url="https://example/api/v1")
    edge_routes = MagicMock()
    mock_client.routes = edge_routes

    api_error_500 = CriblControlPlaneError(message="Boom", body=None, raw_response=MagicMock(status_code=500))
    edge_routes.list_async = AsyncMock(side_effect=api_error_500)

    with pytest.raises(RuntimeError, match="Cribl API error while listing routes"):
        await collect_product_routes(
            mock_client,
...
@pytest.mark.asyncio
async def test_collect_product_routes_network_error(mock_ctx: Context) -> None:
    ...
    stream_routes.list_async = AsyncMock(side_effect=httpx.ConnectError("fail"))

    with pytest.raises(RuntimeError, match="Network error while listing routes"):
        await collect_product_routes(
            mock_client,
            product=ProductsCore.STREAM,
            timeout_ms=10000,
...
@pytest.mark.asyncio
async def test_collect_product_routes_unavailable_product_returns_unavailable(mock_ctx: Context) -> None:
...
@pytest.mark.asyncio
async def test_collect_product_routes_network_error_on_groups(mock_ctx: Context) -> None:
...
@pytest.mark.asyncio
async def test_collect_product_routes_api_error_non_404_on_groups(mock_ctx: Context) -> None:
    ...
    with pytest.raises(RuntimeError, match="Cribl API error while listing stream groups for routes"):
        await collect_product_routes(
            mock_client,
            product=ProductsCore.STREAM,
            timeout_ms=10000,
...
@pytest.mark.asyncio
async def test_collect_product_routes_skips_groups_without_id(mock_ctx: Context) -> None:
    ...
    resp_g1.items[0].model_dump.return_value = {"id": "r1", "name": "main"}
    stream_routes.list_async = AsyncMock(return_value=resp_g1)
