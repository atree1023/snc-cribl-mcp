"""Unit tests for event breaker collection helpers.
...
from snc_cribl_mcp.operations.breakers import collect_product_breakers
...
class TestSerializeModel:
    ...
    def test_serialize_model_with_pydantic_model(self) -> None:
        ...
        mock_obj = MagicMock()
        mock_obj.model_dump.return_value = {"id": "test", "name": "breaker1"}

        result = serialize_model(mock_obj)

        assert result == {"id": "test", "name": "breaker1"}
...
@pytest.mark.asyncio
async def test_collect_product_breakers_success(mock_ctx: Context, security: Security) -> None:
    """It should list breakers for each group via HTTP requests and aggregate results."""
    ...
    resp_g1.json.return_value = {"items": [{"id": "b1"}, {"id": "b2"}], "count": 2}

    resp_g2 = MagicMock()
    resp_g2.status_code = 200
    resp_g2.json.return_value = {"items": [{"id": "b3"}], "count": 1}

    async def http_get(url: str, **_kwargs: object) -> MagicMock:
        if "/m/g1/lib/breakers" in url:
            return resp_g1
        if "/m/g2/lib/breakers" in url:
            return resp_g2
        return MagicMock(status_code=404)

    mock_http_client.get = AsyncMock(side_effect=http_get)

    result = await collect_product_breakers(
        mock_client,
        security,
...
@pytest.mark.asyncio
async def test_collect_product_breakers_404_per_group(mock_ctx: Context, security: Security) -> None:
    """404 on a group's breakers should be treated as empty for that group."""
    mock_client = MagicMock()
...
@pytest.mark.asyncio
async def test_collect_product_breakers_http_error_per_group(mock_ctx: Context, security: Security) -> None:
    """HTTP errors on a group's breakers should raise RuntimeError."""
    mock_client = MagicMock()
    groups_response = MagicMock(items=[MagicMock()])
    groups_response.items[0].model_dump.return_value = {"id": "g1"}
    mock_client.groups.list_async = AsyncMock(return_value=groups_response)
    ...
    with pytest.raises(RuntimeError, match="Network error while listing breakers"):
        await collect_product_breakers(
            mock_client,
            security,
            product=ProductsCore.STREAM,
            timeout_ms=10000,
...
@pytest.mark.asyncio
async def test_collect_product_breakers_unavailable_product(mock_ctx: Context, security: Security) -> None:
...
@pytest.mark.asyncio
async def test_collect_product_breakers_api_error_non_404(mock_ctx: Context, security: Security) -> None:
...
@pytest.mark.asyncio
async def test_collect_product_breakers_network_error_on_groups(mock_ctx: Context, security: Security) -> None:
...
@pytest.mark.asyncio
async def test_collect_product_breakers_skips_groups_without_id(mock_ctx: Context, security: Security) -> None:
    ...
    resp_g1 = MagicMock()
    resp_g1.status_code = 200
    resp_g1.json.return_value = {"items": [{"id": "b1"}], "count": 1}
...
@pytest.mark.asyncio
async def test_collect_product_breakers_with_groupid_field(mock_ctx: Context, security: Security) -> None:
    ...
    resp.json.return_value = {"items": [{"id": "b1"}]}
...
@pytest.mark.asyncio
async def test_collect_product_breakers_without_bearer_auth(mock_ctx: Context) -> None:
    """Test that breakers can be collected even without bearer auth in Security."""
