...
Gets one installed Pack by Pack ID. By default, this returns the Pack metadata plus a bounded summary of the Pack's sources, destinations, pipelines, routes, knowledge categories, and settings categories. Pass `kind` to drill into a section, `object_id` to fetch one object, `detail="full"` to include raw payloads, and `cursor`/`limit` to page large Pack sections.
